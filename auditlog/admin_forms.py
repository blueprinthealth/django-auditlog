...
class AuditChangeAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('timestamp', 'user',
                  'model_type', 'model_pk', 'action', 'pre_change_state', 'changes', 'request')


class RequestLogForm(forms.ModelForm):
    timestamp = forms.DateTimeField()
    http_meta = forms.Field(widget=DictionaryDisplayWidget)
    http_params = forms.Field(widget=DictionaryDisplayWidget)
    data = forms.Field(widget=DictionaryDisplayWidget)
